...
import er.diagram.edit.commands.RegularEntity2CreateCommand;
 ...
 * @generated
 */
public class ISAItemSemanticEditPolicy extends ErBaseItemSemanticEditPolicy {

	/**
	 * @generated
	 */
	public ISAItemSemanticEditPolicy() {
		super(ErElementTypes.ISA_2006);
	}
	...
	protected Command getCreateCommand(CreateElementRequest req) {
		if (ErElementTypes.RegularEntity_3007 == req.getElementType()) {
			return getGEFWrapper(new RegularEntity2CreateCommand(req));
		}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (ErVisualIDRegistry.getVisualID(outgoingLink) == ISASubtypesEditPart.VISUAL_ID) {
			...
			if (ErVisualIDRegistry.getVisualID(outgoingLink) == ISAClassificationAttributeEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						outgoingLink.getSource().getElement(), null,
						outgoingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			case RegularEntity2EditPart.VISUAL_ID:
				for (Iterator<?> it = node.getSourceEdges().iterator(); it
						.hasNext();) {
					Edge outgoingLink = (Edge) it.next();
					if (ErVisualIDRegistry.getVisualID(outgoingLink) == RegularEntityEntityEditPart.VISUAL_ID) {
						DestroyReferenceRequest r = new DestroyReferenceRequest(
								outgoingLink.getSource().getElement(), null,
								outgoingLink.getTarget().getElement(), false);
						cmd.add(new DestroyReferenceCommand(r));
