...
def _filter(profiler):
	return (Filter.checkProfilerAttribute(profiler=profiler,attr='lang',neg=False) and Filter.checkProfilerTags(profiler, extensionList=["it"], negation=False, quantity=1, noMore=False))
Select(name="books",path=_from,where=_filter)
_from = [Step("C:"),Step("Users"),Step("maksim"),Step("Documents"),Step("JuPyteR"),Step("data"),Step("test"),Step("Downloads")]
def _filter(profiler):
	return (Filter.checkProfilerAttribute(profiler=profiler,attr='lang',neg=False) and Filter.checkProfilerTags(profiler, extensionList=["it"], negation=False, quantity=-1, noMore=False) and profiler.getExtension()  in ["mp4","mkv","tlv","gov"])
Select(name="videos",path=_from,where=_filter)
